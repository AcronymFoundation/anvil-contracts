 ...
 */
contract Permit2PassThroughLiquidator is ILiquidator, Ownable2StepUpgradeable, UUPSUpgradeable {
  ...
  /**
   * @notice Initializes this contract, setting the owner, authorized caller, and Permit2 address.
   ...
   * @param _permit2Address The address of the Permit2 contract to use for approvals.
   */
  function initialize(address _owner, address _authorizedCaller, address _permit2Address) external initializer {
    if (_authorizedCaller == address(0) || _permit2Address == address(0)) revert InvalidZeroAddress();
  ...
  function liquidate(
    ...
    // Revoke the target's Permit2 allowance before checking output or refunding unused input. An
    // expiration of zero lasts for the current block in Permit2, so expiration alone would still
    // let an untrusted target spend retained input-token balances after this call returns.
    IPermit2(permit2Address).approve(_inputTokenAddress, addressToApprove, 0, 0);
