 ...
 * @dev Contract Version: 3.0.0
 * @dev Version 3.0.0:
 *  - `tag` on createStaticLOC/createDynamicLOC is dynamic-length bytes (max MAX_TAG_BYTES), emitted
 *    via the new LOCCreatedV3 event.
 *  - Collateral factors are global: the per-LOC collateralFactorBasisPoints /
 *    liquidatorIncentiveBasisPoints snapshots are no longer written (or read) — all health and fee
 *    calculations use the pair's current CollateralFactor config.
 *  - New CollateralFactor field redeemBufferBasisPoints: the fee charged when a HEALTHY LOC's
 *    collateral is liquidated to serve a redemption; unhealthy liquidations continue to pay
 *    liquidatorIncentiveBasisPoints.
 *  - convertLOC() is restricted to UNHEALTHY LOCs: the creator's ability to voluntarily convert a
 *    healthy LOC (and the creator-authorization machinery around it) is removed. Healthy LOCs are
 *    liquidated exclusively through redeemLOC(). The _creatorAuthorization parameter is retained
 *    but ignored so the ILiquidatable ABI is unchanged for existing liquidation infrastructure.
 */
  ...
  SignatureNoncesUpgradeable
{
  ...
  /// The maximum byte length of a LOC tag. Tags are event-only (never stored), so this bounds log
  /// size while allowing multiple concatenated values (e.g. sixteen 32-byte file hashes).
  uint256 public constant MAX_TAG_BYTES = 512;
   ...
   * May be up to MAX_TAG_BYTES bytes (e.g. multiple concatenated identifiers or file hashes).
   */
  function createDynamicLOC(
    ...
    if (_tag.length > MAX_TAG_BYTES) revert TagTooLong(MAX_TAG_BYTES, _tag.length);
  ...
  function extendLOC(uint96 _locId, uint32 _newExpirationTimestamp) external {
    ...
    // NB: pre-V3 this reverted if the pair's liquidator incentive had risen since LOC creation
    // (the incentive was locked in per LOC). Collateral factors are global as of V3 — the current
    // config always applies — so extension no longer changes the LOC's liquidation economics.
  ...
  function modifyLOCCollateral(
    ...
    if (_byAmount <= 0) {
      ...
      uint256 requiredCollateralFactorBasisPoints = collateralToCreditedToCollateralFactors[loc.collateralTokenAddress][
      ...
      uint16 cfBasisPoints = Pricing.collateralFactorInBasisPoints(newCollateralAmount, loc.creditedTokenAmount, price);
   ...
   * @dev As of V3, only unhealthy LOCs may be converted (see the LOCHealthy revert in
   * _liquidateLOCCollateral); healthy LOCs are liquidated exclusively through redeemLOC. The
   * _creatorAuthorization parameter is deprecated and ignored — creators no longer have
   * voluntary-conversion rights, so there is nothing to authorize. It remains in the signature so
   * the ILiquidatable ABI existing liquidation infrastructure depends on is unchanged.
   */
  function convertLOC(
    uint96 _locId,
    address _liquidatorToUse,
    bytes calldata _oraclePriceUpdate,
    bytes calldata /* _creatorAuthorization (deprecated, ignored) */,
    bytes calldata _liquidatorParams
  ) external payable refundExcess nonReentrant {
    LOC memory loc = locs[_locId];

    // NB: _liquidateLOCCollateral trusts its callers to validate; mirror _redeemLOC's checks.
    if (loc.creditedTokenAmount == 0) revert LOCNotFound(_locId);
    if (loc.collateralTokenAddress == loc.creditedTokenAddress) revert LOCAlreadyConverted(_locId);
    if (loc.expirationTimestamp <= block.timestamp) revert LOCExpired(_locId, loc.expirationTimestamp);
  ...
  function upgradePriceOracle(IPriceOracle _newPriceOracle) public onlyOwner {
    ...
    try IERC165(address(_newPriceOracle)).supportsInterface(type(IPriceOracle).interfaceId) returns (bool supported) {
      if (!supported) revert InvalidUpgradeContract();
    } catch (bytes memory /*lowLevelData*/) {
      revert InvalidUpgradeContract();
    }
  ...
  function _persistAndEmitNewLOCCreated(
    ...
    locs[_locId] = LOC(
      ...
      _beneficiary,
      _expirationTimestamp,
      0, // Deprecated per-LOC collateralFactorBasisPoints; factors are global as of V3.
      0, // Deprecated per-LOC liquidatorIncentiveBasisPoints; factors are global as of V3.
      collateralContract,
      _collateralTokenAddress,
      _collateralTokenAmount,
  ...
  function _markLOCPartiallyLiquidated(
    ...
    storedLoc.claimableCollateral = _loc.claimableCollateral - _liquidationContext.collateralToClaimAndSendLiquidator;
   ...
   * @dev Only reachable from _redeemLOC and convertLOC, which perform LOC and caller validation
   * before calling (LOC exists, is not expired, and its collateral is not already the credited
   * asset; redemption is beneficiary-authorized with a non-zero destination).
   * @param _locId The ID of the LOC in question.
   * @param _loc The LOC in question, as loaded by the caller.
   ...
   * @param _redeemDestinationAddress The address to which the redeemed credited token will be sent,
   * or the zero address when called from convertLOC (conversion, not redemption).
  ...
  function _liquidateLOCCollateral(
    ...
    bytes calldata _liquidatorParams,
    bytes calldata _oraclePriceUpdate,
    address _redeemDestinationAddress
  ) private returns (uint256 _collateralUsed, uint256 _claimableCollateralUsed) {
    address collateralTokenAddress = _loc.collateralTokenAddress;
    address creditedTokenAddress = _loc.creditedTokenAddress;
    bool partialRedeem = _requiredCreditedAmount != _loc.creditedTokenAmount;
    ...
    // Healthy LOCs may only be liquidated to serve a redemption. NB: unlike pre-V3, this applies
    // to the creator too — voluntary conversion of a healthy LOC is no longer possible.
    if (!liquidationContext.locUnhealthy && _redeemDestinationAddress == address(0)) revert LOCHealthy(_locId);

    if (
      liquidationContext.collateralToClaimAndSendLiquidator == 0 || liquidationContext.creditedTokenAmountToReceive == 0
    ) {
    ...
    address liquidatorAddress;
    {
      ...
      if (_iLiquidatorToUse != address(0)) {
        ...
        {
          (uint256 collateralRemaining, ) = _loc.collateralContract.claimCollateral(
            _loc.collateralId,
            _claimableCollateralUsed,
            address(this),
            !partialRedeem
          );
          _collateralUsed = _loc.collateralTokenAmount - collateralRemaining;
        }
        ...
        // NB: utilizing scope to free stack space and prevent "Stack too deep" compile error.
        {
          ...
          uint256 receivedAmount = IERC20(creditedTokenAddress).balanceOf(address(this)) - creditedBalanceBefore;
          if (receivedAmount != liquidationContext.creditedTokenAmountToReceive)
            revert ConversionFundsReceivedMismatch(liquidationContext.creditedTokenAmountToReceive, receivedAmount);
        }
      } else {
        ...
        if (_redeemDestinationAddress != msg.sender) {
    ...
    if (partialRedeem) {
      _markLOCPartiallyLiquidated(_locId, msg.sender, liquidatorAddress, _collateralUsed, _loc, liquidationContext);
    } else {
      _markLOCConverted(_locId, msg.sender, liquidatorAddress, _loc, liquidationContext);
    }
  }
  ...
  function _calculateLiquidationContext(
    ...
    // NB: collateral factors are global as of V3: read the pair's current config, never the values
    // stored on the LOC (those are 0 for V3-created LOCs and stale snapshots for older ones).
    CollateralFactor memory collateralFactor = collateralToCreditedToCollateralFactors[_loc.collateralTokenAddress][
      _loc.creditedTokenAddress
    ];
    ...
    /*** Determine if this LOC is insolvent, and if so, adjust credited amount to receive. ***/
    {
      ...
      uint256 maxCreditedTokenAmountToReceive = Pricing.amountBeforeFee(
        ...
        collateralFactor.liquidatorIncentiveBasisPoints
      );
      ...
      if (maxCreditedTokenAmountToReceive < _loc.creditedTokenAmount) {
        ...
        // NB: insolvent implies unhealthy, so the liquidator incentive (not the redeem buffer) applies.
        return
          _createLiquidationContextUsingAllClaimableCollateral(
            _loc,
    ...
    bool locUnhealthy = (_loc.creditedTokenAmount * 10_000) / collateralAmountInCreditedToken >=
      collateralFactor.collateralFactorBasisPoints;

    // A healthy LOC is only ever liquidated to serve a redemption (_liquidateLOCCollateral reverts
    // with LOCHealthy on the conversion path otherwise), so the redeem buffer is the fee taken from
    // its collateral; unhealthy liquidations pay the full liquidator incentive.
    uint16 feeBasisPoints = locUnhealthy
      ? collateralFactor.liquidatorIncentiveBasisPoints
      : collateralFactor.redeemBufferBasisPoints;

    uint256 liquidationAmount = (_loc.collateralTokenAmount * _requiredCreditedAmount) /
      collateralAmountInCreditedToken;

    uint256 liquidatorFeeAmount = Pricing.percentageOf(liquidationAmount, feeBasisPoints);
   ...
   * @param _liquidatorIncentiveBasisPoints The pair's current (global) liquidator incentive.
   * @return The LiquidationContext.
   */
  function _createLiquidationContextUsingAllClaimableCollateral(
    LOC memory _loc,
    uint256 _creditedToReceive,
    uint16 _liquidatorIncentiveBasisPoints
  ) private pure returns (LiquidationContext memory) {
    uint256 collateralToClaim = _loc.claimableCollateral;
    uint256 collateralToTrade = Pricing.amountBeforeFee(collateralToClaim, _liquidatorIncentiveBasisPoints);
    return
      LiquidationContext(
  ...
  function _upsertCollateralFactorsAsOwner(AssetPairCollateralFactor[] memory _assetPairCollateralFactors) private {
    for (uint256 i = 0; i < _assetPairCollateralFactors.length; ++i) {
      ...
      // The redeem buffer is a fee taken from collateral just like the liquidator incentive, so it
      // is bounded the same way.
      uint16 redeemBufferBasisPoints = cf.redeemBufferBasisPoints;
      if (redeemBufferBasisPoints > maxLiquidatorIncentive)
        revert RedeemBufferAboveMax(maxLiquidatorIncentive, redeemBufferBasisPoints);
      ...
      emit CollateralFactorUpdatedV2(
        collateralAddress,
        creditedAddress,
